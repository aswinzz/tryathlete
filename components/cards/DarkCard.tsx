  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 20 }}>
        ...
        {heroUnit && (
          <span style={{ fontFamily: "system-ui, sans-serif", fontSize: 24, fontWeight: 700, color: TEXT2, marginBottom: 4 }}>
            {heroUnit}
          </span>
        )}
      </div>
        ...
        <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 12, marginTop: 4 }}>
          ...
          {laps.map((lap) => {
            ...
            return (
              <div
                key={lap.lapIndex}
                style={{ display: "grid", gridTemplateColumns: "3fr 2.5fr 1fr 2.5fr 2fr", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${BORDER}` }}
              >
                <span style={{ fontFamily: "system-ui, sans-serif", fontSize: 11, color: TEXT, display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  {isFastest && (
                    <span style={{ background: ACCENT, color: "#000", fontSize: 7, fontWeight: 800, padding: "2px 5px", borderRadius: 4 }}>
                      FAST
                    </span>
                  )}
                </span>
      ...
      {/* Footer */}
      {calories && (
        <div style={{ marginTop: 20, paddingTop: 14, borderTop: `1px solid ${BORDER}`, display: "flex", justifyContent: "flex-end" }}>
          <span style={{ fontFamily: "system-ui, sans-serif", fontSize: 10, color: TEXT3 }}>{calories.toLocaleString()} KCAL</span>
        </div>
      )}
    </div>
  );
