...
export function NightCard({
  ...
  return (
    <div ref={cardRef} style={{ background: glass ? "transparent" : BG, borderRadius: 20, overflow: "hidden", padding: "24px 22px 28px", width: "100%", boxSizing: "border-box", position: "relative", border: `1px solid ${bdr}` }}>
      {!glass && (
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 6, marginBottom: 20 }}>
        <span style={{ fontFamily: "system-ui, sans-serif", fontSize: 64, fontWeight: 900, color: TEXT, lineHeight: 1, textShadow: glass ? "0 2px 10px rgba(0,0,0,0.7)" : undefined }}>{heroValue}</span>
      ...
      {config.show.route && routePoints && routePoints.length > 1 && (
        glass ? (
          <div style={{ marginBottom: 14 }}>
            <RouteMapSvg routePoints={routePoints} viewW={400} viewH={200} padding={20} strokeColor={MAP_STROKE} strokeWidth={2.5} glowOpacity={0.45} glowWidth={12} style={{ filter: "drop-shadow(0 0 5px rgba(129,140,248,0.4)) drop-shadow(0 0 3px rgba(0,0,0,0.8))" }} />
          </div>
        ) : (
