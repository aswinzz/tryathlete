...
export function RetroCard({
  ...
  if (isRun) {
    ...
    if (calories) stats.push({ label: "CALORIES", value: `${calories.toLocaleString()} KCAL` });
    if (elevGain) stats.push({ label: "ELEVATION", value: `+${Math.round(elevGain)} M` });
  } else if (isCycle) {
    stats = [
      { label: "AVG SPEED", value: avgPace ? `${formatSpeed(1 / avgPace)} KM/H` : "—" },
      { label: "DURATION", value: formatDuration(duration) },
      { label: "ELEV GAIN", value: elevGain ? `+${Math.round(elevGain)} M` : "—" },
    ];
    if (calories) stats.push({ label: "CALORIES", value: `${calories.toLocaleString()} KCAL` });
  } else if (isSwim) {
    stats = [
    ...
    stats = [
      { label: "DURATION", value: formatDuration(duration) },
      { label: "AVG HR", value: avgHeartRate ? `${avgHeartRate} BPM` : "—" },
      { label: "MAX HR", value: maxHeartRate ? `${maxHeartRate} BPM` : "—" },
  ...
  return (
    ...
    >
      ...
      <div style={{ textAlign: "center", borderBottom: `2px solid ${TEXT}`, paddingBottom: 10, marginBottom: 12 }}>
        ...
        <p style={{ ...SANS, fontSize: 22, fontWeight: 900, color: TEXT, letterSpacing: "0.05em", lineHeight: 1 }}>
          {typeLabel}
        </p>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
        ...
        <div style={{ borderRight: `1px solid ${BORDER}`, paddingRight: 20 }}>
          <p style={{ ...SANS, fontSize: 8, fontWeight: 700, color: TEXT2, letterSpacing: "0.14em", textTransform: "uppercase", marginBottom: 6 }}>
            {isEndurance ? "TOTAL DISTANCE" : "TOTAL DURATION"}
          </p>
          <p style={{ ...MONO, fontSize: 48, fontWeight: 700, color: TEXT, lineHeight: 1, letterSpacing: "-0.02em" }}>
            {heroValue}
          </p>
          {heroUnit && (
            <p style={{ ...SANS, fontSize: 14, fontWeight: 700, color: TEXT2, marginTop: 4 }}>{heroUnit}</p>
          )}
        </div>
      ...
      {laps.length > 0 && (
        <>
          ...
          <p style={{ ...SANS, fontSize: 7, fontWeight: 700, color: TEXT2, letterSpacing: "0.22em", textTransform: "uppercase", marginBottom: 8 }}>
            SPLITS
          </p>
          {/* Header */}
      ...
      <div style={{ borderTop: `2px solid ${TEXT}`, marginTop: 16, paddingTop: 8 }}>
        <p style={{ ...SANS, fontSize: 7, color: TEXT3, textAlign: "center", letterSpacing: "0.12em" }}>
          — ALL RIGHTS RESERVED —
        </p>
      </div>
    </div>
